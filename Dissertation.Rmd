...
pkgs <- c(
  ...
  "reshape",     # to exapand a df and create histograms
  "gridExtra",   # to arrange two plots next to each other on the same row
  "stplanr",     # to perform transport-related operations, eg. OD creatio between two centroids
  "rgdal", 
  "ggpubr",      # to create pie charts
  "tidyverse",   # to include other useful packages like tibble
  "ipfp",        # to perform IPF
...
**Figure 2.1:** Areas of microsimualtion studies during the period 1967-2003   [Source: [@ballas_spatial_2009]]
...
**Figure 2.2:** Schematic diagram of population synthesis using spatial microsimulation  [Source: [@crooks_agent-based_2017]]
...
```{r, fig.height=3, out.extra='style="background-color: #C0C0C0; padding:1px; display: inline-block;"', echo=FALSE}
img31=readPNG("pics/Chapter3/Methodology.png")
grid.raster(img31)

```

**Figure 3.1:** Methodology for the allocation of individuals to OD pairs [Source: Self-composed]
...
**Figure 3.2:** Map of Yorkshire and the Humber region [Source: Self-composed]
...
```{r, echo=FALSE, message=F, warning=F, fig.hold='hold', fig.align='center', fig.width=50, fig.height=20}
...
```{r, fig.height=4, out.extra='style="background-color: #C0C0C0; padding:1px; display: inline-block;"', echo=FALSE}
img34=readPNG("pics/Chapter3/OD_age_methodology.png")
grid.raster(img34)

```

**Figure 3.4:** Illustration of the cleaning algorithm for the OD per Age dataset [Source: Self-composed]
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results=FALSE, fig.height=5, out.extra='style="background-color: #C0C0C0; padding:1px; display: inline-block;"'}
...
**Figure 3.5:** Destination-Workplace types of the employed population [Source: Self-composed]
...
**Figure 3.6:** Commuting trip generation [Source: Self-composed]
...
**Figure 3.7:** Commuting trip attraction [Source: Self-composed]
...
**Figure 3.8:** Commuting trips with destination in Leeds city centre [Source: Self-composed]
...
**Figure 3.9:** Commuting trips with destination in Hull city centre [Source: Self-composed]
...
**Figure 3.10:** Commuting trips with destination in York city centre [Source: Self-composed]
...
**Figure 3.11:** Commuting trips with destination in Bradford city centre [Source: Self-composed]
...
Based on the Census data, taking the city centre of Leeds as an example, car commuting trips (*Figure 3.12*) have a larger radius than bus trips (*Figure 3.14*). Specifically, car trips, as depicted in *Figure 3.13*, have a median distance of 9.51km, bus trips seem to be around a certain buffer from the examined zone, with a median distance of 5.34km (*Figure 3.15*). Cycling trips (*Figure 3.16*) have a median distance of 4.96km (*Figure 3.17*) and finally pedestrian trips (*Figure 3.18*) have a far less median distance of 2.37km, as expected (*Figure 3.19*). From the last plot of pedestrian trip distances, various reporting errors, previously mentioned in *Chapter 2* and characteristic of traditional surveys, can be seen, such as the great number of pedestrian trips at distances more than 10km. Even if the distance measured refers only to the straight distance between zone centroids, the majority of those trips should be at a distance that prohibits the full journey to be completed by walking and are probably a result of the respondent’s misreporting or misunderstanding. (*For the code to create these maps and plots see Code/Chapter3/OD_pairs.R*)
...
**Figure 3.12:** Car commuting trips attracted to Leeds city centre [Source: Self-composed]
...
**Figure 3.13:** Car commuting trips and OD distance [Source: Self-composed]
...
**Figure 3.14:** Bus commuting trips attracted to Leeds city centre [Source: Self-composed]
...
**Figure 3.15:** Bus commuting trips and OD distance [Source: Self-composed]
...
**Figure 3.16:** Cycling commuting trips attracted to Leeds city centre [Source: Self-composed]
...
**Figure 3.17:** Cycling commuting trips and OD distance [Source: Self-composed]
...
**Figure 3.18:** Pedestrian commuting trips attracted to Leeds city centre [Source: Self-composed]
...
**Figure 3.19:** Pedestrian commuting trips and OD distance [Source: Self-composed]
...
```{r, fig.height=.5, out.extra='style="background-color: #C0C0C0; padding:1px; display: inline-block;"', echo=FALSE}
img41=readPNG("pics/Chapter4/ipf_syntax.png")
grid.raster(img41)

```

**Figure 4.1:** R syntax for the implementation of IPF [Source: [@lovelace_spatial_2016]
...
**Figure 4.2:** R-syntax for the TRS algorithm [Source: [@lovelace_spatial_2016]
...
The second part of validation, external validation, was performed using the “Hours worked per week” as the target variable and comparing the simulated values with the real ones. As it can be seen from *Table 4.4* (*see Code/Chapter4/IPF_fitStats.R*), the fit statistics are worse compared to the constraint variables, as expected. Nonetheless, even if this variable was not used in the IPF, its simulated values are close enough to the real ones *(corr>0,8)*, except from the first level, which contains the least total values. Scatter plots for each variable level are also presented in *Appendix III (Figures A.38-A.41)*.
...
**Figure 4.4:** Spatial distribution of employed individuals with Social Grade A-B [Self-composed]
...
**Figure 4.5:** Spatial distribution of employed individuals with Social Grade C1 [Self-composed]
...
**Figure 4.6:** Spatial distribution of employed individuals with Social Grade C2 [Self-composed]
...
**Figure 4.7:** Spatial distribution of employed individuals with Social Grade D-E [Self-composed]
...
```{r, fig.height=1.5, out.extra='style="background-color: #C0C0C0; padding:1px; display: inline-block;"', echo=FALSE}
img48=readPNG("pics/Chapter4/ipf_OD.png")
grid.raster(img48)

```

**Figure 4.8:** R-syntax for the implementation of IPF per Origin zone [Source: Self-composed;  [@lovelace_spatial_2016]]
...
**Figure 4.10:** Origin-Residence (green)-Destination-Workplace (orange) of the first 5 individuals [Source: Self-composed]
...
**Figure 4.11:** Destination zones with commuters of Social Grade A-B [Source: Self-composed]
...
**Figure 4.12:** Destination zones with commuters of Social Grade C1 [Source: Self-composed]
...
**Figure 4.13:** Destination zones with commuters of Social Grade C2 [Source: Self-composed]
...
**Figure 4.14:** Destination zones with commuters of Social Grade D-E [Source: Self-composed]
...
**Figure 4.15:** Mode of travel for the first 5 individuals [Source: Self-composed]
...
**Figure 5.1:** Social grade distribution of affected car and PT commuters [Source: Self-composed]
...
**Figure 5.2:** Desire lines of car commuters with Social Grade A-B and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.3:** Desire lines of car commuters with Social Grade C1 and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.4:** Desire lines of car commuters with Social Grade C2 and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.5:** Desire lines of car commuters with Social Grade D-E and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.6:** Desire lines of PT commuters with Social Grade A-B and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.7:** Desire lines of PT commuters with Social Grade C1 and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.8:** Desire lines of PT commuters with Social Grade C2 and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.9:** Desire lines of PT commuters with Social Grade D-E and a destination in the Leeds city centre [Source: Self-composed]
...
**Figure 5.10:** Affected car commuters (Losers) with Social Grade A-B [Source: Self-composed]
...
**Figure 5.11:** Affected car commuters (Losers) with Social Grade C1 [Source: Self-composed]
...
**Figure 5.12:** Affected car commuters (Losers) with Social Grade C2 [Source: Self-composed]
...
**Figure 5.13:** Affected car commuters (Losers) with Social Grade D-E [Source: Self-composed]
...
**Figure 5.14:** Affected PT commuters (Winners) with Social Grade A-B [Source: Self-composed]
...
**Figure 5.15:** Affected PT commuters (Winners) with Social Grade C1 [Source: Self-composed]
...
**Figure 5.16:** Affected PT commuters (Winners) with Social Grade C2 [Source: Self-composed]
...
**Figure 5.17:** Affected PT commuters (Winners) with Social Grade D-E [Source: Self-composed]

## Individual-level analysis

In addition to the aggregated analysis, the OD microdata also provides the ability for a similar type of analysis at a greater level of detail, e.g. for specific individuals. For that purpose, two randomly selected individuals, a car commuter (car passenger) and a public transport commuter (train user), are presented in *Figure 5.18*, along with some of their sociodemographic characteristics. As it is depicted, the first individual is a young female car user (*“Gender” 2*) of age between 25-34 years old (*“Age” 3*), social grade of D or E (*“Approximated Social Grade” 4*) and she usually between 31 to 48 hours per week (*“Hours worked per week” 3*) at retail (*“Industry” 4*). On the other hand, the second commuter is an older female bus user (*“Gender” 2*) of age between 45-54 (*“Age” 5*), social grade of A or B (*“Approximated Social Grade” 1*) working part-time of around 16-30 hours per week (*“Hours worked per week” 2*) in activities related to education (*“Industry” 10*). Looking at the 2 sampled individuals, it is obvious that the proposed scheme would benefit a wealthy individual at the expense of a younger one with a possible low income. Specifically, the first individual with an estimated net annual income of £10000 (*see Figure A.53*) would have to pay an extra amount of £500 per year for 250 working days (*£2x250 working days-excluding weekends and bank holidays*), which is 2.8% of her total income and would need to double her transport budget from £2x250=£500 to £4x250=£1000. Similar insights at the individual level have the potential to provide a clear understanding of the repercussions of a certain policy and give the ability to prepare additional complementary measures to minimize the negative impacts.
...
**Figure 5.18:** Desire lines, travel mode and main sociodemographic characteristics for the two sampled individuals affected from the scheme [Source: Self-composed]
